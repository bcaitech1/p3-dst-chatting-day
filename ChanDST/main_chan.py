...
def eval_guid():
    test_dials = json.load(open("/opt/ml/input/data/eval_dataset/eval_dials.json"))
    guids = []
    for dials in test_dials:
        guid = []
        dial_idx = dials['dialogue_idx']
        for i in range((len(dials['dialogue'])//2)):
            guid.append(f"{dial_idx}-{i}")
        guids.append(guid)
    return guids
...
def recover_state(pred_slot):
    ...
    slot = json.load(open("/opt/ml/input/data/eval_dataset/slot_meta.json"))
    label_map = [{i: label for i, label in enumerate(labels)} for labels in label_list]
    prediction = []
    for i in range(45):
        if label_map[i][int(pred_slot[i])] != 'none':
            prediction.append(f"{slot[i]}-{label_map[i][int(pred_slot[i])]}")
    return prediction
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir",
                        default='/opt/ml/p3-dst-chatting-day/result',
                        type=str,
    ...
    output_model_file = "/opt/ml/p3-dst-chatting-day/CHAN-DST/model/loss.best"
    model = BeliefTracker(args, num_labels, device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for idx, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
            ...
            with torch.no_grad():
                pred_slot = model(input_ids, input_len, labels = None)
                if not os.path.exists(args.output_dir):
                    os.mkdir(args.output_dir)
                for i in range(pred_slot.size(0)): # 16
                    for j in range(len(eval_guid[i + (idx * args.eval_batch_size)])): # turn 개수만큼 pred_slot예측 해줄거
                        prediction[eval_guid[i + (idx * args.eval_batch_size)][j]] = recover_state(pred_slot[i][j])
        json.dump(
            prediction,
            open(f"output_model.csv", "w"),
            indent=2,
            ensure_ascii=False,
        )
