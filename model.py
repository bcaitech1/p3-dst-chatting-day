...
from modeling_bert import CustomBertPreTrainedModel, CustomBertModel, CustomBertLMPredictionHead, CustomBertPreTrainingHeads
...
class TransformerDST(CustomBertPreTrainedModel):
    def __init__(self, config, dec_config, n_op, n_domain,
        ...
        self.bert = CustomBertModel(config, type_vocab_size)

        # predictor
        self.encoder = TransformerEncoder(config, self.bert, n_op, n_domain, update_id, exclude_domain)
...
class TransformerEncoder(nn.Module):
    def __init__(self, config, bert, n_op, n_domain, update_id, exclude_domain=False):
        ...
        assert isinstance(bert, CustomBertModel)
...
class BertForSeq2SeqDecoder(nn.Module):
    ...
    def __init__(self, config, dec_config, bert, bert_model_embedding_weights, mask_word_id, eos_id, pad_id):
        ...
        self.predictions = CustomBertLMPredictionHead(
