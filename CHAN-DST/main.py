...
class Processor(DataProcessor):
    ...
    def __init__(self, config):
        ...
        # MultiWOZ dataset
        elif "multiwoz" in config.data_dir:
    ...
    def __init__(self, examples, label_list, tokenizer, max_seq_length=64, max_turn_length=22):
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            if prev_dialogue_idx is None or prev_turn_idx < max_turn_length:
                features.append(
                    InputFeatures(input_ids=input_ids, # 문장들 encode 한
                                  input_len=input_len, # 문장 a,b 길이
                                  label_id=label_id, #35개의 slot에 대한 label
                                  update=list(map(int, example.update)))) # update된 label이 뭔지
...
def main():
    ...
    if args.do_train: # args.do_train
